...
const latestRfidFirmware="1.9.0";
...
const firmwareNeedsUpdate=(value?:string|null)=>value?compareFirmware(value,latestRfidFirmware)<0:false;
...
export function RfidDevicePanel(){
  ...
  return <div className="rfid-admin-shell">
    ...
    <section className="panel rfid-device-panel">
      ...
      <div className={`rfid-ble-runtime ${runtimeStatus.state}`}><span><IconBluetooth size={20}/></span><div><strong>{runtimeStatus.state==="online"?"Bluetooth-Leser bereit":runtimeStatus.state==="connecting"?"Bluetooth wird verbunden":"Bluetooth-Status"}</strong><small>{runtimeStatus.message}</small></div><i/></div>
      ...
      <div className="rfid-device-list">{devices.map(device=>{
        ...
        const firmwareState=!device.firmwareVersion
          ...
          :compareFirmware(device.firmwareVersion,latestRfidFirmware)>0
            ?`Firmware ${device.firmwareVersion} · neuer als App`
            :`Firmware ${device.firmwareVersion}${needsUpdate?" · Update verfügbar":" · aktuell"}`;
        return <article key={device.id} className={device.active?"":"inactive"}><span><IconNfc size={22}/></span><div><strong>{device.name}</strong><small>{device.hardwareId?`${device.hardwareId} · `:""}{device.lastSeenAt?`Zuletzt verbunden: ${new Date(device.lastSeenAt).toLocaleString("de-DE")}`:isBleDevice?"Noch keine aktive Bluetooth-Sitzung":"Noch kein Scan empfangen"}</small><small className="rfid-firmware-state">{firmwareState}</small></div><div className="rfid-device-actions">{isBleDevice&&!isBleOnline&&<button className="connect" disabled={!device.active||bleSearchBusy||bleBusy} onClick={searchBleReader}><IconBluetooth size={17}/>{bleSearchBusy?"Android-Auswahl offen …":"Bluetooth verbinden"}</button>}<button className="firmware" disabled={!device.active||!needsUpdate||Boolean(updating)||Boolean(restarting)||isBleDevice&&!isBleOnline} onClick={()=>updateFirmware(device)}><IconRefresh size={17}/>{updating===device.id?"Update läuft …":"Firmware aktualisieren"}</button><button className="restart" disabled={!device.active||Boolean(restarting)||Boolean(updating)||isBleDevice&&!isBleOnline} onClick={()=>restart(device)}><IconRefresh size={17}/>{restarting===device.id?"Wird neu gestartet …":"Neu starten"}</button><button onClick={()=>toggle(device)}>{device.active?"Deaktivieren":"Aktivieren"}</button></div></article>;
    ...
    <section className="panel rfid-setup-panel">
      ...
      {setupMode==="esp32"&&<div className="rfid-setup-content" role="tabpanel">
        <div className="rfid-ble-setup">
          ...
          <section className={`rfid-ble-config-step ${bleReader?"ready":"locked"}`} aria-labelledby="rfid-ble-config-title"><div className="rfid-ble-step-title"><b>2</b><div><strong id="rfid-ble-config-title">Mit ClubIQ verknüpfen</strong><small>{registeredBleOnline?"Bereits registriert und einsatzbereit":bleReader?`${bleReader.name} ausgewählt`:"Zuerst oben einen BLE-Leser auswählen"}</small></div></div><div className="rfid-ble-fields single"><label>Name des Lesers<input value={bleName} maxLength={60} autoComplete="off" disabled={!bleReader||registeredBleOnline} onChange={event=>setBleName(event.target.value)} placeholder="z. B. Leser am Tresen"/></label></div><div className="rfid-ble-direct-note"><IconCheck size={18}/><span><strong>{registeredBleOnline?"Leser ist einsatzbereit":"Keine WLAN-Daten am Leser nötig"}</strong><small>{registeredBleOnline?"Du kannst zur Kasse zurückkehren und sofort eine Karte auflegen.":"Scans, Display, Chip-Schreiben und Updates laufen direkt über das Tablet."}</small></span></div><button className="rfid-ble-connect" disabled={registeredBleOnline||!bleReader||bleSupport!=="ready"||bleBusy||bleSearchBusy||bleName.trim().length<3} onClick={provisionBle}><IconCheck size={20}/>{registeredBleOnline?"Bereits mit ClubIQ verbunden":bleBusy?"Leser wird verknüpft …":"Nur neuen Leser mit ClubIQ verknüpfen"}</button></section>
