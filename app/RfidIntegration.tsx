...
export function RfidDevicePanel(){
  ...
  const [bleName,setBleName]=useState("RFID-Leser Vereinsheim"),[bleSsid,setBleSsid]=useState(()=>typeof localStorage==="undefined"?"":localStorage.getItem("clubiq-rfid-wifi-ssid")||""),[blePassword,setBlePassword]=useState(""),[bleBusy,setBleBusy]=useState(false),[bleState,setBleState]=useState("idle"),[bleMessage,setBleMessage]=useState("Leser einschalten und in der Nähe des Tablets bereithalten.");
  const [setupMode,setSetupMode]=useState<"esp8266"|"esp32">("esp8266"),[bleSupport,setBleSupport]=useState<"checking"|"ready"|"unavailable">("checking");
  ...
  const updateCount=devices.filter(device=>device.firmwareVersion&&device.firmwareVersion!==latestRfidFirmware).length;
  ...
  return <div className="rfid-admin-shell">
    ...
    <section className="panel rfid-device-panel">
      ...
      <div className="rfid-flow-strip"><span>RFID-Leser</span><b>→</b><span>Raspberry</span><b>→</b><span>Tablet-Kasse</span></div>
    ...
    <section className="panel rfid-setup-panel">
      ...
      <div className="rfid-setup-picker" role="tablist" aria-label="RFID-Gerät auswählen">
        <button role="tab" aria-selected={setupMode==="esp8266"} className={setupMode==="esp8266"?"active":""} onClick={()=>setSetupMode("esp8266")}><span><IconWifi size={25}/></span><div><strong>Vorhandener ESP8266</strong><small>NodeMCU V3 mit deinem bisherigen Leser-WLAN</small></div></button>
        <button role="tab" aria-selected={setupMode==="esp32"} className={setupMode==="esp32"?"active":""} onClick={()=>setSetupMode("esp32")}><span><IconBluetooth size={25}/></span><div><strong>Neuer ESP32</strong><small>Direkt per App verbinden · Bluetooth am Android-Tablet</small></div></button>
      </div>
      ...
      {setupMode==="esp32"&&<div className="rfid-setup-content" role="tabpanel">
        <div className="rfid-ble-setup">
          <div className="rfid-ble-heading"><span><IconBluetooth size={24}/></span><div><strong>Neuen ESP32-Leser verbinden</strong><small>Gerät auswählen, Vereins-WLAN eintragen und anschließend mit ClubIQ verknüpfen.</small></div><em className={bleSupport}>{bleSupport==="ready"?"Bluetooth bereit":bleSupport==="checking"?"Wird geprüft":"Browser ungeeignet"}</em></div>
          ...
          <section className="rfid-ble-device-step" aria-labelledby="rfid-ble-device-title"><div className="rfid-ble-step-title"><b>1</b><div><strong id="rfid-ble-device-title">BLE-Leser auswählen</strong><small>Auch ein bereits in Android gekoppeltes Gerät muss einmal für ClubIQ freigegeben werden.</small></div></div>{bleReaders.length>0&&<div className="rfid-ble-reader-list">{bleReaders.map(reader=><button key={reader.id} className={bleReader?.id===reader.id?"selected":""} onClick={()=>{setBleReader(reader);setBleState("selected");setBleMessage(`${reader.name} ist für die Verknüpfung ausgewählt.`)}}><span><IconBluetooth size={20}/></span><div><strong>{reader.name}</strong><small>{bleReader?.id===reader.id?"Ausgewählt":"Bereits für diese App freigegeben"}</small></div>{bleReader?.id===reader.id&&<IconCheck size={20}/>}</button>)}</div>}<button className="rfid-ble-search" disabled={bleSupport!=="ready"||bleSearchBusy||bleBusy} onClick={searchBleReader}><IconBluetooth size={20}/>{bleSearchBusy?"BLE-Geräte werden gesucht …":"BLE-Gerät suchen und auswählen"}</button><small className="rfid-ble-browser-note">Chrome öffnet eine geschützte Geräteliste. Dort <b>ClubIQ-RFID-…</b> auswählen und „Koppeln“ bestätigen.</small></section>
          <section className={`rfid-ble-config-step ${bleReader?"ready":"locked"}`} aria-labelledby="rfid-ble-config-title"><div className="rfid-ble-step-title"><b>2</b><div><strong id="rfid-ble-config-title">Mit ClubIQ verknüpfen</strong><small>{bleReader?`${bleReader.name} ausgewählt`:"Zuerst oben einen BLE-Leser auswählen"}</small></div></div><div className="rfid-ble-fields"><label>Name des Lesers<input value={bleName} maxLength={60} autoComplete="off" disabled={!bleReader} onChange={event=>setBleName(event.target.value)} placeholder="z. B. Leser am Tresen"/></label><label>Vereins-WLAN (2,4 GHz)<input value={bleSsid} maxLength={32} autoComplete="username" disabled={!bleReader} onChange={event=>setBleSsid(event.target.value)} placeholder="WLAN-Name"/></label><label>WLAN-Kennwort<input type="password" value={blePassword} maxLength={63} autoComplete="new-password" disabled={!bleReader} onChange={event=>setBlePassword(event.target.value)} placeholder="Mindestens 8 Zeichen"/></label></div><button className="rfid-ble-connect" disabled={!bleReader||bleSupport!=="ready"||bleBusy||bleSearchBusy||bleName.trim().length<3||!bleSsid.trim()||blePassword.length<8} onClick={provisionBle}><IconCheck size={20}/>{bleBusy?"Leser wird verknüpft …":"Ausgewählten Leser mit ClubIQ verknüpfen"}</button></section>
    ...
    <details className="panel rfid-collapsible rfid-advanced"><summary><span><strong>Erweiterte Einrichtung</strong><small>Notfallweg mit manuellem Geräte-Token und technische Hinweise</small></span></summary><div className="rfid-security-note"><strong>Die Karte speichert weder Geld noch Berechtigungen.</strong><small>Die UID erkennt nur die Person. Kontostände, Preise und Zugriffsrechte werden ausschließlich in PostgreSQL geprüft.</small></div><div className="rfid-device-create"><label>Bezeichnung<input value={name} maxLength={60} onChange={event=>setName(event.target.value)} placeholder="z. B. Leser am Tresen"/></label><button disabled={busy||name.trim().length<3} onClick={create}>Token erzeugen</button></div>{token&&<div className="rfid-token-once"><div><strong>Gerätekennung – nur jetzt sichtbar</strong><small>Notfallweg, falls die normale Kopplung nicht möglich ist.</small></div><div><code>{token}</code><button onClick={copyToken}>{copied?<IconCheck size={18}/>:<IconCopy size={18}/>} {copied?"Kopiert":"Kopieren"}</button></div></div>}<details className="rfid-network-help"><summary>Warum läuft der Scan über den Raspberry?</summary><p>Das Tablet würde beim direkten Wechsel zu 192.168.4.1 seine Kassenverbindung verlieren. Im normalen Betrieb senden Leser und Tablet deshalb verschlüsselt über den Raspberry; das Leser-WLAN bleibt nur der Einrichtungs- und Notfallweg.</p></details></details>
  </div>;
}
