...
const latestRfidFirmware="1.8.0";
    ...
    <section className="panel rfid-setup-panel">
      ...
      {setupMode==="esp32"&&<div className="rfid-setup-content" role="tabpanel">
        <div className="rfid-ble-setup">
          ...
          <section className={`rfid-ble-config-step ${bleReader?"ready":"locked"}`} aria-labelledby="rfid-ble-config-title"><div className="rfid-ble-step-title"><b>2</b><div><strong id="rfid-ble-config-title">Mit ClubIQ verknüpfen</strong><small>{bleReader?`${bleReader.name} ausgewählt`:"Zuerst oben einen BLE-Leser auswählen"}</small></div></div><div className="rfid-ble-fields single"><label>Name des Lesers<input value={bleName} maxLength={60} autoComplete="off" disabled={!bleReader} onChange={event=>setBleName(event.target.value)} placeholder="z. B. Leser am Tresen"/></label></div><div className="rfid-ble-direct-note"><IconCheck size={18}/><span><strong>Keine WLAN-Daten am Leser nötig</strong><small>Scans, Display, Chip-Schreiben und Updates laufen direkt über das Tablet.</small></span></div><button className="rfid-ble-connect" disabled={!bleReader||bleSupport!=="ready"||bleBusy||bleSearchBusy||bleName.trim().length<3} onClick={provisionBle}><IconCheck size={20}/>{bleBusy?"Leser wird verknüpft …":"Ausgewählten Leser mit ClubIQ verknüpfen"}</button></section>
