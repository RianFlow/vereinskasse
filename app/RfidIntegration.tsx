...
type Device={id:string;name:string;hardwareId?:string|null;active:boolean|number;lastSeenAt?:string|null;createdAt:string};
...
export function RfidDevicePanel(){
  ...
  const [name,setName]=useState("RFID-Leser Vereinsheim"),[token,setToken]=useState(""),[error,setError]=useState(""),[notice,setNotice]=useState(""),[busy,setBusy]=useState(false),[pairingBusy,setPairingBusy]=useState<string|null>(null),[restarting,setRestarting]=useState<string|null>(null),[copied,setCopied]=useState(false);
  ...
  return <section className="panel rfid-device-panel"><div className="panel-head"><div><p className="eyebrow">EXTERNER KARTENLESER</p><h2>ESP8266 + MFRC522</h2></div><span><IconWifi size={20}/> Ohne PC koppeln</span></div>
