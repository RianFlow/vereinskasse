...
import { IconAlertCircle, IconBluetooth, IconCheck, IconNfc, IconRefresh, IconWifi } from "@tabler/icons-react";
...
export function RfidDevicePanel(){
  ...
  const [bleSupport,setBleSupport]=useState<"checking"|"ready"|"unavailable">("checking");
  ...
  const load=async()=>{try{const response=await fetch("/api/rfid/devices",{cache:"no-store"}),data=await response.json();if(!response.ok)throw new Error(data.error);setDevices(data.devices||[]);setCards(data.cards||[])}catch(reason){setError(reason instanceof Error?reason.message:"RFID-Leser konnten nicht geladen werden")}};
  ...
  useEffect(()=>{if(bleSupport!=="ready")return;let stopped=false;void getAuthorizedRfidBleReaders().then(readers=>{if(stopped)return;setBleReaders(readers);if(readers.length===1)setBleReader(current=>current||readers[0])}).catch(()=>undefined);return()=>{stopped=true}},[bleSupport]);
  ...
  return <div className="rfid-admin-shell">
    ...
    <section className="panel rfid-device-panel">
      ...
      })}{!devices.length&&<div className="rfid-empty-state"><IconNfc size={30}/><strong>Noch kein RFID-Leser verbunden</strong><small>Richte unten einen ESP32 D1 mini einmalig per Bluetooth ein.</small></div>}</div>
    </section>

    <section className="panel rfid-setup-panel">
      <div className="panel-head"><div><p className="eyebrow">ESP32 D1 MINI</p><h2>RFID-Leser einrichten</h2><small>Bluetooth überträgt nur die Einstellungen. Danach arbeitet der Leser dauerhaft über das Kassen-WLAN.</small></div></div>
      <div className="rfid-setup-content">
        <div className="rfid-ble-setup">
          <div className="rfid-ble-heading"><span><IconWifi size={24}/></span><div><strong>ESP32 D1 mini ins Kassen-WLAN aufnehmen</strong><small>Der Leser startet nach dem Speichern neu. Die App bestätigt den Erfolg erst, wenn der Raspberry ihn wirklich erreicht.</small></div><em className={bleSupport}>{bleSupport==="ready"?"Einrichtung bereit":bleSupport==="checking"?"Wird geprüft":"Browser ungeeignet"}</em></div>
          ...
          <section className="rfid-ble-device-step" aria-labelledby="rfid-ble-device-title"><div className="rfid-ble-step-title"><b>1</b><div><strong id="rfid-ble-device-title">Leser auswählen</strong><small>Android zeigt einmal seine geschützte Bluetooth-Gerätefreigabe.</small></div></div>{bleReaders.length>0&&<div className="rfid-ble-reader-list">{bleReaders.map(reader=><button key={reader.id} className={bleReader?.id===reader.id?"selected":""} onClick={()=>{setBleReader(reader);setBleState("selected");setBleMessage(`${reader.name} ist für die WLAN-Einrichtung ausgewählt.`)}}><span><IconBluetooth size={20}/></span><div><strong>{reader.name}</strong><small>{bleReader?.id===reader.id?"Ausgewählt":"Bereits für diese App freigegeben"}</small></div>{bleReader?.id===reader.id&&<IconCheck size={20}/>}</button>)}</div>}<button className="rfid-ble-search" disabled={bleSupport!=="ready"||bleSearchBusy||bleBusy} onClick={searchBleReader}><IconBluetooth size={20}/>{bleSearchBusy?"Android-Auswahl ist geöffnet …":"ESP32 D1 mini auswählen"}</button><small className="rfid-ble-browser-note">Dort <b>ClubIQ-RFID-…</b> antippen. Bei einer späteren WLAN-Änderung fordert der Leser zur Sicherheit eine RFID-Karte direkt am Gerät an.</small></section>
          <section className={`rfid-ble-config-step ${bleReader?"ready":"locked"}`} aria-labelledby="rfid-ble-config-title"><div className="rfid-ble-step-title"><b>2</b><div><strong id="rfid-ble-config-title">Kassen-WLAN speichern</strong><small>{selectedRegisteredDevice?"Registrierter Leser · WLAN kann sicher geändert werden":bleReader?`${bleReader.name} ausgewählt`:"Zuerst oben einen Leser auswählen"}</small></div></div><div className="rfid-ble-fields"><label>Name des Lesers<input value={bleName} maxLength={60} autoComplete="off" disabled={!bleReader} onChange={event=>setBleName(event.target.value)} placeholder="z. B. Leser am Tresen"/></label><label>WLAN-Name<input value={kioskSsid} maxLength={32} autoCapitalize="none" autoComplete="off" disabled={!bleReader} onChange={event=>setKioskSsid(event.target.value)} placeholder="ClubIQ-Kasse"/></label><label>WLAN-Kennwort<input type="password" value={kioskPassword} minLength={12} maxLength={63} autoComplete="new-password" disabled={!bleReader} onChange={event=>setKioskPassword(event.target.value)} placeholder="Kennwort des Kassen-WLANs"/></label></div><div className="rfid-ble-direct-note"><IconCheck size={18}/><span><strong>Stabiler WLAN-Betrieb</strong><small>Bluetooth und WLAN laufen nie gleichzeitig. Nach dem Neustart übernimmt nur das feste 2,4-GHz-Kassen-WLAN.</small></span></div><button className="rfid-ble-connect" disabled={!bleReader||bleSupport!=="ready"||bleBusy||bleSearchBusy||bleName.trim().length<3||kioskSsid.trim().length<3||kioskPassword.length<12} onClick={provisionBle}><IconCheck size={20}/>{bleBusy?"Einrichtung wird geprüft …":selectedRegisteredDevice?"WLAN dieses Lesers aktualisieren":"Leser ins Kassen-WLAN aufnehmen"}</button></section>
