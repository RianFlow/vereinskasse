...
export function RfidDevicePanel(){
  ...
  const [bleName,setBleName]=useState("RFID-Leser Vereinsheim"),[bleSsid,setBleSsid]=useState(()=>typeof localStorage==="undefined"?"":localStorage.getItem("clubiq-rfid-wifi-ssid")||""),[blePassword,setBlePassword]=useState(""),[bleBusy,setBleBusy]=useState(false),[bleState,setBleState]=useState("idle"),[bleMessage,setBleMessage]=useState("Leser mit Strom versorgen, dann auf Einfach verbinden tippen."),[showAdvanced,setShowAdvanced]=useState(!Boolean(typeof localStorage==="undefined"?"":localStorage.getItem("clubiq-rfid-wifi-ssid")||""));
  ...
  const provisionBle=async()=>{if(bleBusy||bleName.trim().length<3||!bleSsid.trim())return;setBleBusy(true);setError("");setNotice("");setBleState("searching");try{await provisionRfidReader({name:bleName.trim(),ssid:bleSsid.trim(),password:blePassword},progress=>{setBleState(progress.state);setBleMessage(progress.message)});localStorage.setItem("clubiq-rfid-wifi-ssid",bleSsid.trim());setBlePassword("");setNotice(`${bleName.trim()} ist jetzt verbunden und sofort einsatzbereit.`);await load()}catch(reason){setBleState("error");setBleMessage(reason instanceof Error?reason.message:"Bluetooth-Einrichtung fehlgeschlagen")}finally{setBleBusy(false)}};
  ...
  return <section className="panel rfid-device-panel"><div className="panel-head"><div><p className="eyebrow">EXTERNER KARTENLESER</p><h2>RFID-Leser</h2></div><span><IconBluetooth size={20}/> Einfach verbinden</span></div>
    <div className="rfid-architecture"><IconNfc size={28}/><div><strong>Einfach verbinden</strong><small>Leser einschalten, auf „Einfach verbinden“ tippen und fertig. ClubIQ übernimmt WLAN, Zertifikat und Kopplung automatisch.</small></div></div>
    ...
    <div className="rfid-ble-setup"><div className="rfid-ble-heading"><span><IconBluetooth size={24}/></span><div><strong>Neuen ESP32-Leser verbinden</strong><small>Bluetooth am Tablet einschalten, den Leser mit Strom versorgen und dann direkt verbinden.</small></div></div>{(showAdvanced||!bleSsid.trim())?<div className="rfid-ble-fields"><label>Name des Lesers<input value={bleName} maxLength={60} autoComplete="off" onChange={event=>setBleName(event.target.value)} placeholder="z. B. Leser am Tresen"/></label><label>Vereins-WLAN (2,4 GHz)<input value={bleSsid} maxLength={32} autoComplete="username" onChange={event=>setBleSsid(event.target.value)} placeholder="WLAN-Name"/></label><label>WLAN-Kennwort<input type="password" value={blePassword} maxLength={63} autoComplete="new-password" onChange={event=>setBlePassword(event.target.value)} placeholder="Optional bei offenem WLAN"/></label></div>:<div><strong>{bleSsid.trim()||"Vereins-WLAN"}</strong><small>Die gespeicherten WLAN-Daten werden direkt verwendet. Für andere Daten einfach „WLAN ändern“ wählen.</small></div>}<div className="rfid-ble-actions"><button className="rfid-ble-connect" disabled={bleBusy||bleName.trim().length<3||!bleSsid.trim()} onClick={provisionBle}><IconBluetooth size={20}/>{bleBusy?"Wird verbunden …":"Einfach verbinden"}</button>{bleSsid.trim()&&<button type="button" className="secondary" disabled={bleBusy} onClick={()=>setShowAdvanced(current=>!current)}>{showAdvanced?"Gespeicherte Daten nutzen":"WLAN ändern"}</button>}</div><div className={`rfid-ble-progress ${bleState}`} role="status"><span>{bleState==="approved"?<IconCheck size={20}/>:bleState==="error"?<IconAlertCircle size={20}/>:<IconRefresh size={20}/>}</span><p>{bleMessage}</p></div></div>
    <details className="rfid-network-help"><summary>So läuft die Verbindung ab</summary><ol><li>Der Leser wird direkt über Bluetooth gefunden.</li><li>ClubIQ überträgt WLAN, Zertifikat und die sichere Freigabe.</li><li>Der Leser meldet sich anschließend selbstständig beim Server an.</li></ol></details>
