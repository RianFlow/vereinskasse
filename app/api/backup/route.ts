...
const COMPATIBLE_SCHEMA_VERSIONS=[22,23,24,SCHEMA_VERSION];
...
export async function GET(request:Request){
  ...
  const runtimeEnv=env as unknown as {VEREINSKASSE_RUNTIME?:string;VEREINSKASSE_DATABASE_PROVIDER?:string},runtime=runtimeEnv.VEREINSKASSE_RUNTIME==="raspberry"?"raspberry":"cloud",databaseProvider=runtime==="raspberry"?(runtimeEnv.VEREINSKASSE_DATABASE_PROVIDER||"sqlite"):"d1";
  return Response.json({healthy:true,runtime,databaseProvider,schemaVersion:SCHEMA_VERSION,backupFormatVersion:BACKUP_FORMAT_VERSION,database:{sales:sales?.count||0,members:members?.count||0,transactions:transactions?.count||0,profiles:profiles?.count||0},snapshots,restoreRequests:requests.results.map(row=>({...row,preview:JSON.parse(String(row.previewJson||"{}"))}))},{headers:{"cache-control":"no-store"}});
}
