...
import { members, products, sales } from "../../../db/schema";
...
export async function GET() {
  try { await ensureSeed(); const db=getDb(); return Response.json({ products:await db.select().from(products), members:await db.select().from(members), sales:await db.select().from(sales).orderBy(desc(sales.time)).limit(500) }); }
...
export async function POST(request:Request) {
  try { const body=await request.json() as {id:string,total:number,items:number,time:string,member:string,memberId:string,method:string,cart:unknown};
    ...
    await getDb().insert(sales).values({id:body.id,total:body.total,items:body.items,time:body.time,member:body.member,memberId:body.memberId,method:body.method,cartJson:JSON.stringify(body.cart),backupKey}).onConflictDoNothing();
