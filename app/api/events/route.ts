...
type EventRow={id:string;name:string;startsAt:string;endsAt:string|null;status:string;notes:string|null;productIdsJson:string|null;createdBy:string;salesCount:number;itemsCount:number;revenue:number};
type ProductRow={productId:number;productName:string;quantity:number;total:number};
const parseProductIds=(value:string|null)=>{if(value===null)return null;try{const parsed=JSON.parse(value);return Array.isArray(parsed)?parsed.filter(id=>Number.isInteger(id)).map(Number):[]}catch{return []}};
const presentEvent=({productIdsJson,...event}:EventRow)=>({...event,productIds:parseProductIds(productIdsJson)});

const listSql=`SELECT e.id,e.name,e.starts_at startsAt,e.ends_at endsAt,e.status,e.notes,e.product_ids_json productIdsJson,e.created_by createdBy,
...
export async function GET(request:Request){
  try{
    ...
    const storedEvents=await env.DB.prepare(listSql).bind(profile.id).all<EventRow>(),events=storedEvents.results.map(presentEvent);
    if(!id)return Response.json({events});
    const selected=events.find(e=>e.id===id);if(!selected)return Response.json({error:"Veranstaltung nicht gefunden"},{status:404});
    ...
    const previous=events.filter(e=>e.startsAt<selected.startsAt&&e.id!==id).sort((a,b)=>b.startsAt.localeCompare(a.startsAt))[0]||null;
...
export async function POST(request:Request){
  try{
    ...
    const body=await request.json() as {name?:string;notes?:string;productIds?:unknown};const name=(body.name||"").trim().slice(0,80);if(name.length<3)return Response.json({error:"Bitte einen Veranstaltungsnamen eingeben"},{status:400});
    ...
    const requested=Array.isArray(body.productIds)?[...new Set(body.productIds.map(Number))]:[];
    if(!requested.length||requested.length>200||requested.some(id=>!Number.isInteger(id)))return Response.json({error:"Bitte mindestens einen Artikel für die Veranstaltung auswählen"},{status:400});
    const available=await env.DB.prepare("SELECT id FROM products WHERE profile_id=?").bind(profile.id).all<{id:number}>(),allowed=new Set(available.results.map(product=>Number(product.id)));
    if(requested.some(id=>!allowed.has(id)))return Response.json({error:"Ein ausgewählter Artikel ist nicht mehr im Sortiment. Bitte Auswahl aktualisieren."},{status:409});
    const id=`EVENT-${crypto.randomUUID()}`,now=new Date().toISOString(),notes=(body.notes||"").trim().slice(0,300)||null,productIdsJson=JSON.stringify(requested);
    await env.DB.batch([env.DB.prepare("INSERT INTO events (id,profile_id,name,starts_at,status,notes,product_ids_json,created_by,created_at) VALUES (?,?,?,?,'active',?,?,?,?)").bind(id,profile.id,name,now,notes,productIdsJson,operator.id,now),env.DB.prepare("INSERT INTO audit_logs (id,action,entity_type,entity_id,operator_id,details_json,created_at) VALUES (?,?,?,?,?,?,?)").bind(crypto.randomUUID(),"EVENT_CREATED","event",id,operator.id,JSON.stringify({profileId:profile.id,name,productIds:requested}),now)]);
    await env.BACKUPS.put(`events/${now.slice(0,10)}/${id}.json`,JSON.stringify({id,name,notes,productIds:requested,status:"active",createdBy:operator,createdAt:now}),{httpMetadata:{contentType:"application/json"}});
    return Response.json({ok:true,event:{id,name,startsAt:now,endsAt:null,status:"active",notes,productIds:requested,createdBy:operator.id,salesCount:0,itemsCount:0,revenue:0}},{status:201});
