...
const listSql=`SELECT e.id,e.name,e.starts_at startsAt,e.ends_at endsAt,e.status,e.notes,e.created_by createdBy,
  ...
  FROM events e ORDER BY CASE WHEN e.status='active' THEN 0 ELSE 1 END,e.starts_at DESC`;

export async function GET(request:Request){
  try{
    const id=new URL(request.url).searchParams.get("id");
    const events=await env.DB.prepare(listSql).all<EventRow>();
...
export async function POST(request:Request){
  try{
    const operator=await requireRole(request,["Vorstand","Kassendienst"]);if(!operator)return Response.json({error:"Keine Berechtigung"},{status:403});
    ...
    if(await env.DB.prepare("SELECT id FROM events WHERE status='active' LIMIT 1").first())return Response.json({error:"Bitte zuerst die laufende Veranstaltung beenden"},{status:409});
    const id=`EVENT-${crypto.randomUUID()}`,now=new Date().toISOString(),notes=(body.notes||"").trim().slice(0,300)||null;
    await env.DB.batch([env.DB.prepare("INSERT INTO events (id,name,starts_at,status,notes,created_by,created_at) VALUES (?,?,?,'active',?,?,?)").bind(id,name,now,notes,operator.id,now),env.DB.prepare("INSERT INTO audit_logs (id,action,entity_type,entity_id,operator_id,details_json,created_at) VALUES (?,?,?,?,?,?,?)").bind(crypto.randomUUID(),"EVENT_CREATED","event",id,operator.id,JSON.stringify({name}),now)]);
...
export async function PATCH(request:Request){
  try{
    ...
    const event=await env.DB.prepare("SELECT id,name,status FROM events WHERE id=?").bind(body.id).first<{id:string;name:string;status:string}>();if(!event)return Response.json({error:"Veranstaltung nicht gefunden"},{status:404});
    if(body.action==="reopen"&&await env.DB.prepare("SELECT id FROM events WHERE status='active' AND id<>? LIMIT 1").bind(body.id).first())return Response.json({error:"Es läuft bereits eine andere Veranstaltung"},{status:409});
    const now=new Date().toISOString(),status=body.action==="close"?"closed":"active",endsAt=body.action==="close"?now:null;
    await env.DB.batch([env.DB.prepare("UPDATE events SET status=?,ends_at=? WHERE id=?").bind(status,endsAt,body.id),env.DB.prepare("INSERT INTO audit_logs (id,action,entity_type,entity_id,operator_id,details_json,created_at) VALUES (?,?,?,?,?,?,?)").bind(crypto.randomUUID(),body.action==="close"?"EVENT_CLOSED":"EVENT_REOPENED","event",body.id,operator.id,JSON.stringify({name:event.name}),now)]);
