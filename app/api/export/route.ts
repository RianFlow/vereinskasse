...
export async function GET(request:Request){
  ...
  const rows=await env.DB.prepare("SELECT s.id,s.time,s.member,s.member_id memberId,s.method,s.items,s.total,e.name eventName,p.tendered,p.change_due changeDue,COALESCE((SELECT GROUP_CONCAT(si.quantity || 'x ' || si.product_name, ' | ') FROM sale_items si WHERE si.sale_id=s.id),'') details FROM sales s LEFT JOIN events e ON e.id=s.event_id LEFT JOIN payments p ON p.sale_id=s.id WHERE s.profile_id=? ORDER BY s.time DESC").bind(profile.id).all<Record<string,unknown>>();
