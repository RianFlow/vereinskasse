...
export async function GET(request:Request){
  ...
  if(!admin||!profile)return Response.json({error:"Nur der Vorstand darf RFID-Leser verwalten"},{status:403,headers:noStore});
...
export async function DELETE(request:Request){
  try{
    const [admin,profile]=await Promise.all([requireRole(request,["Vorstand"]),requireProfile(request)]);
    if(!admin||!profile)return Response.json({error:"Nur der Vorstand darf Kartenzuordnungen entfernen"},{status:403,headers:noStore});
    ...
    const card=await env.DB.prepare("SELECT member_id memberId FROM rfid_cards WHERE profile_id=? AND uid=?").bind(profile.id,uid).first<{memberId:string}>();
