...
export async function POST(request:Request){
  try{
    ...
    const body=await request.json() as {state?:unknown;customerName?:unknown;itemsText?:unknown;itemCount?:unknown;totalCents?:unknown};
    ...
    const itemsText=String(body.itemsText||"").trim().slice(0,100)||null;
    ...
    await env.DB.prepare("INSERT INTO rfid_display_states (profile_id,state,customer_name,items_text,item_count,total_cents,revision,updated_at) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(profile_id) DO UPDATE SET state=excluded.state,customer_name=excluded.customer_name,items_text=excluded.items_text,item_count=excluded.item_count,total_cents=excluded.total_cents,revision=excluded.revision,updated_at=excluded.updated_at")
      .bind(profile.id,state,customerName,state==="cart"?itemsText:null,state==="cart"?itemCount:0,state==="cart"?totalCents:0,revision,now).run();
