...
export async function PATCH(request:Request){
  try{const body=await request.json() as {roundId:string;memberId:string;memberName:string}; if(!body.roundId||!body.memberId)return Response.json({error:"Angaben fehlen"},{status:400});
