...
export async function PATCH(request:Request){
  try{const body=await request.json() as {roundId:string;memberId:string;memberName:string};const [member,profile]=await Promise.all([sessionUser(request),requireProfile(request)]);if(!member||member.id!==body.memberId||!profile)return Response.json({error:"Mitglied und Profil müssen identifiziert sein"},{status:403});if(!body.roundId||!body.memberId)return Response.json({error:"Angaben fehlen"},{status:400});
    const reduced=await env.DB.prepare("UPDATE rounds SET remaining=remaining-1, active=CASE WHEN remaining-1<=0 THEN 0 ELSE 1 END WHERE id=? AND profile_id=? AND active=1 AND remaining>0 AND (SELECT COALESCE(SUM(quantity),0) FROM round_claims WHERE profile_id=? AND round_id=? AND member_id=?) < max_per_member").bind(body.roundId,profile.id,profile.id,body.roundId,body.memberId).run();
    if(!reduced.meta.changes)return Response.json({error:"Limit erreicht oder Runde beendet"},{status:409});
    const claimId=`${body.roundId}-${body.memberId}`; const claimedAt=new Date().toISOString(); await env.DB.prepare("INSERT INTO round_claims (id,profile_id,round_id,member_id,member_name,quantity,claimed_at) VALUES (?,?,?,?,?,1,?) ON CONFLICT(id) DO UPDATE SET quantity=quantity+1, claimed_at=excluded.claimed_at").bind(claimId,profile.id,body.roundId,body.memberId,body.memberName,claimedAt).run();
    await env.BACKUPS.put(`round-claims/${profile.id}/${claimedAt.slice(0,10)}/${body.roundId}-${body.memberId}-${Date.now()}.json`,JSON.stringify({...body,profileId:profile.id,quantity:1,claimedAt}),{httpMetadata:{contentType:"application/json"}});
