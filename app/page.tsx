...
type GuestAccount={id:string;name:string;type:"visitor"|"club";active?:boolean};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/api/data").then(r => { if(!r.ok) throw new Error(); return r.json(); }).then(data => {
      setProducts(data.products || seed);setDiscounts(data.discounts||[]);setClubMembers(data.members||members);setGuestAccounts(data.guests||[]);const remembered=sessionStorage.getItem("vereinskasse-active-member");if(remembered)setActiveMember((data.members||members).find((m:Member)=>m.id===remembered)||null); setSales(data.sales || []); setRounds(data.rounds || []); setStorageState("online"); loadControl();
  ...
  const finishCheckout = async(allocations: Allocation[], round?:RoundSpec, payment?:{tendered:number;changeDue:number}, guestAccount?:GuestAccount) => {
    if(!operator) return;
    const sale = { id: crypto.randomUUID(), total, items: itemCount, time: new Date().toISOString(), member: operator.name, memberId: operator.id, method: identify || "Mitgliedskonto", cart, lines:currentLines(), allocations, round, guestAccount, ...payment };
    ...
    if(guestAccount){setSelectedGuest(guestAccount);setGuestAccounts(g=>[guestAccount,...g.filter(x=>x.id!==guestAccount.id)])}if(payment)setChangeToast(payment.changeDue);setIdentify(null); setOperator(null);
  ...
  return <MembersContext.Provider value={clubMembers}><main className={`app ${darkMode ? "dark" : "light"}`}>
    ...
    {view === "kasse" ? <section className="pos">
      <div className="catalog">
        ...
        {operationMode==="event"&&<div className={`selected-customer ${selectedGuest?"":"empty-customer"}`}><span className="avatar">{selectedGuest?(selectedGuest.type==="club"?"V":"G"):"1"}</span><div><strong>{selectedGuest?.name||"Besucher oder Verein wählen"}</strong><small>{selectedGuest?`${selectedGuest.type==="club"?"Besucherverein":"Besucher"} · Einkäufe können gesammelt werden`:"Optional für Anschreiben · Barverkauf geht auch ohne Auswahl"}</small></div>{selectedGuest&&<div className="balance"><small>Aktuell offen</small><b>{money(Number(control.accounts.find(a=>a.memberId===selectedGuest.id)?.balance||0))}</b></div>}<button onClick={()=>setGuestFinder(true)}>{selectedGuest?"Ändern":"Anlegen / wählen"}</button></div>}
      ...
      {operator && identify==="Tageskonto" && <GuestAccountDialog total={total} operator={operator} selected={selectedGuest} onClose={() => {setOperator(null);setIdentify(null)}} onConfirm={(a,g)=>finishCheckout(a,undefined,undefined,g)} />}
    ...
    {guestFinder&&<GuestFinder guests={guestAccounts} current={selectedGuest} onClose={()=>setGuestFinder(false)} onSelect={g=>{setSelectedGuest(g);setGuestFinder(false)}}/>}
...
function GuestFinder({guests,current,onClose,onSelect}:{guests:GuestAccount[];current:GuestAccount|null;onClose:()=>void;onSelect:(g:GuestAccount)=>void}){const [query,setQuery]=useState("");const [type,setType]=useState<"visitor"|"club">("visitor");const [name,setName]=useState("");const filtered=guests.filter(g=>g.active!==false&&g.name.toLocaleLowerCase("de-DE").includes(query.trim().toLocaleLowerCase("de-DE")));const create=()=>{const clean=name.trim().replace(/[^a-zA-Z0-9äöüÄÖÜß .&'\-]/g,"").slice(0,70);if(!clean)return;onSelect({id:`GAST-${crypto.randomUUID().slice(0,8).toUpperCase()}`,name:clean,type})};return <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="identity-card finder-card guest-finder"><button className="modal-close" onClick={onClose}>×</button><p className="eyebrow">VERANSTALTUNGSKONTO</p><h2>Besucher oder Verein</h2><label className="member-search"><span>⌕</span><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="Angelegte Gäste durchsuchen"/><small>{filtered.length} Treffer</small></label><div className="guest-account-list">{filtered.map(g=><button key={g.id} className={current?.id===g.id?"active":""} onClick={()=>onSelect(g)}><span>{g.type==="club"?"V":"G"}</span><div><strong>{g.name}</strong><small>{g.type==="club"?"Besucherverein":"Besucher"}</small></div><b>{current?.id===g.id?"✓":"Auswählen"}</b></button>)}{!filtered.length&&guests.length>0&&<p>Kein passender Eintrag gefunden.</p>}</div><div className="new-guest"><strong>Neu anlegen</strong><div className="guest-type"><button className={type==="visitor"?"active":""} onClick={()=>setType("visitor")}>Einzelner Besucher</button><button className={type==="club"?"active":""} onClick={()=>setType("club")}>Besucherverein</button></div><label>Name<input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")create()}} placeholder={type==="club"?"z. B. SV Musterstadt":"z. B. Familie Meier"}/></label><button className="confirm-allocation" disabled={!name.trim()} onClick={create}>Anlegen und auswählen</button></div></div></div>}

function GuestAccountDialog({total,operator,selected,onClose,onConfirm}:{total:number;operator:Member;selected:GuestAccount|null;onClose:()=>void;onConfirm:(a:Allocation[],g:GuestAccount)=>void}){
  const [name,setName]=useState(selected?.name||"");const [type,setType]=useState<"visitor"|"club">(selected?.type||"visitor");const clean=name.trim().replace(/[^a-zA-Z0-9äöüÄÖÜß .&'\-]/g,"").slice(0,70);const guest:GuestAccount={id:selected?.id||`GAST-${crypto.randomUUID().slice(0,8).toUpperCase()}`,name:clean,type};
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="identity-card guest-card"><button className="modal-close" onClick={onClose}>×</button><p className="eyebrow">SPÄTER BEZAHLEN</p><h2>Auf Gast anschreiben</h2><p className="identity-sub">Die {money(total)} werden gesammelt und können am Tagesende ganz oder teilweise bezahlt werden. Freigegeben durch {operator.name}.</p><div className="guest-type"><button className={type==="visitor"?"active":""} onClick={()=>setType("visitor")}>Besucher</button><button className={type==="club"?"active":""} onClick={()=>setType("club")}>Besucherverein</button></div><label>Name<input autoFocus value={name} onChange={e=>setName(e.target.value)} placeholder={type==="club"?"z. B. SV Musterstadt":"z. B. Familie Meier"}/></label><div className="guest-summary"><span>Wird gebucht auf</span><strong>{clean||"–"}</strong><b>{money(total)}</b></div><button className="confirm-allocation" disabled={!clean} onClick={()=>onConfirm([{memberId:guest.id,memberName:guest.name,amount:total,kind:"anteil"}],guest)}>Auf {clean||"Gast"} buchen</button></div></div>
}
