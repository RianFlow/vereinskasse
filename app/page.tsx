...
export default function Home() {
  ...
  const finishMemberCheckout=()=>{if(!activeMember||!itemCount)return;const sale={id:crypto.randomUUID(),total,items:itemCount,time:new Date().toISOString(),member:activeMember.name,memberId:activeMember.id,method:"Mitgliedskonto",cart,allocations:[{memberId:activeMember.id,memberName:activeMember.name,amount:total,kind:"anteil"}]};setSales(s=>[sale,...s]);fetch("/api/data",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(sale)}).then(r=>{if(!r.ok)throw new Error();setStorageState("online")}).catch(()=>{const pending=JSON.parse(localStorage.getItem("vereinskasse-pending")||"[]");pending.push(sale);localStorage.setItem("vereinskasse-pending",JSON.stringify(pending));setStorageState("offline")});setCart({});setPaid(true);setTimeout(()=>setPaid(false),3500)};
  ...
  return <main className={`app ${darkMode ? "dark" : "light"}`}>
    ...
    {view === "kasse" ? <section className="pos">
      ...
      <aside className="cart">
        ...
        <div className="checkout"><div className="sum"><span>Gesamt <small>{itemCount} Artikel</small></span><strong>{money(total)}</strong></div>{activeMember?<><div className="checkout-person"><span>{activeMember.initials}</span><div><small>Dieser Einkauf geht an</small><strong>{activeMember.name}</strong></div><button onClick={()=>setMemberPrompt(true)}>Ändern</button></div><button className="pay account-direct" disabled={!itemCount} onClick={finishMemberCheckout}>✓ Auf mein Konto buchen</button></>:<button className="pay account-login" disabled={!itemCount} onClick={()=>setMemberPrompt(true)}>◉ Mitglied anmelden</button>}<div className="payment-divider"><span>oder anders abrechnen</span></div><button className="pay cash" disabled={!itemCount} onClick={() => checkout("Bar")}>💶 Bar bezahlen</button><button className="pay paypal" disabled={!itemCount} onClick={() => checkout("PayPal")}>PayPal <span>Demo</span></button><button className="pay split-pay" disabled={!itemCount} onClick={() => checkout("Mitgliedskonto")}>👥 Aufteilen / andere Person</button><button className="pay guest-pay" disabled={!itemCount} onClick={() => checkout("Tageskonto")}>🎟 Gast- oder Tageskonto</button></div>
      </aside>
