...
export default function Home() {
  ...
  const add = (id: number) => { setPaid(false);setChangeToast(null); setCart(c => ({ ...c, [id]: (c[id] || 0) + 1 })); };
  ...
  const finishMemberCheckout=async()=>{if(!billingMember||!itemCount)return;const sale={id:crypto.randomUUID(),total,items:itemCount,time:new Date().toISOString(),member:billingMember.name,memberId:billingMember.id,method:listMember?"Vertrauensliste":"Mitgliedskonto",cart,lines:currentLines(),allocations:[{memberId:billingMember.id,memberName:billingMember.name,amount:total,kind:"anteil"}]};if(!await submitSale(sale))return;setSales(s=>[sale,...s]);const recent=[billingMember.id,...recentMemberIds.filter(id=>id!==billingMember.id)].slice(0,5);setRecentMemberIds(recent);localStorage.setItem("vereinskasse-recent-members",JSON.stringify(recent));setCart({});setPaid(true);setListMember(null);setTimeout(()=>setPaid(false),3500)};
  const finishCheckout = async(allocations: Allocation[], round?:RoundSpec, payment?:{tendered:number;changeDue:number}) => {
    ...
    if(!await submitSale(sale))return;setSales(s => [sale, ...s]);
