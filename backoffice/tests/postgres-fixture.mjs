...
export async function postgresFixture(){
  ...
  if(process.env.BACKOFFICE_TEST_DATABASE_URL){
    ...
    const native=new Pool({connectionString:url.href,max:1});
    // SET ROLE is session-local. Pool.query discards a connection after an
    // expected permission error, so keep this isolated test session leased.
    const client=await native.connect(),query=client.query.bind(client);
    pool={query,connect:async()=>({query,release(){},on:client.on.bind(client),off:client.off.bind(client)})};
    close=async()=>{client.release();await native.end();};
