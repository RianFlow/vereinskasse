...
admin_password_file="${PGADMINPASSWORD_FILE:-/run/secrets/postgres_admin_password}"
app_password_file="${POSTGRES_APP_PASSWORD_FILE:-/run/secrets/postgres_app_password}"

[[ -r "$admin_password_file" ]] || {
  echo "PostgreSQL-Admin-Kennwort fehlt." >&2
  exit 1
}
[[ -r "$app_password_file" ]] || {
...
psql \
  ...
  --port="${PGPORT:-5432}" \
  --username="${PGADMINUSER:-clubiq_admin}" \
  --dbname=postgres \
  --set=ON_ERROR_STOP=1 \
