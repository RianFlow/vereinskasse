...
<script>let pin=sessionStorage.getItem('clubiq-maintenance-pin')||'',networks=[];const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.json(200, {"app": app_ready(), "docker": active("docker"), "wifi": KIOSK_CONNECTION in wifi.stdout, "neighbors": count, "lan": bool(lan.stdout.strip()), "internetWifi": internet_wifi, "uplink": uplink, "backup": backup_ok, "usb": usb.returncode == 0, "diskFree": f"{usage.free / 1024**3:.1f} GB", "diskPercent": round(usage.free / usage.total * 100)})

    def do_POST(self):
        if self.path not in ("/api/action", "/api/internet-wifi/connect"):
            self.json(404, {"error": "Nicht gefunden"})
            return
        if not self.allowed():
            self.json(401, {"error": "Wartungs-PIN falsch oder vorübergehend gesperrt"})
