...
async function saveWifi(){try{let ssid=$('wifiSsid').value.trim();if(!ssid)throw Error('Bitte einen WLAN-Namen eingeben.');let j=await call('/api/wifi',{ssid,password:$('wifiPassword').value,_csrf:csrf,confirm:'SPEICHERN'});$('wifiPassword').value='';$('wifiOut').textContent=`${j.ssid} gespeichert. Verbindung wird aufgebaut.`;setTimeout(refreshStatus,1000)}catch(e){$('wifiOut').textContent=e.message}}
async function deleteWifi(){if(!confirm('Gespeichertes Vereins-WLAN wirklich entfernen?'))return;try{await call('/api/wifi',{_csrf:csrf,confirm:'LOESCHEN'},'DELETE');$('wifiSsid').value='';$('wifiPassword').value='';$('wifiOut').textContent='Vereins-WLAN entfernt. Wartungszugang bleibt aktiv.';setTimeout(refreshStatus,500)}catch(e){$('wifiOut').textContent=e.message}}
