...
test("stellt vergessene Profil-PINs im 2-aus-3-Verfahren wieder her",async()=>{
  ...
  assert.ok(backup.includes("SCHEMA_VERSION=23")&&backup.includes('"profile_recovery_keys"'));
});
...
test("verwendet das Clubiq-Logo und den Slogan ohne das Kassentheme umzubauen",async()=>{
  ...
  for(const feature of ["APP_VERSION","APP_DEVELOPER","APP_COPYRIGHT_YEAR","1.3.0","Florian Hümmling"])assert.ok(info.includes(feature),`${feature} fehlt`);
  ...
  assert.ok(packageFile.includes('"version": "1.3.0"'),"Paketversion stimmt nicht mit der sichtbaren Version überein");
...
test("legt Mitglieder schlank mit kombinierbaren geschützten Funktionen an",async()=>{
  ...
  for(const feature of [".member-section-tabs",".whatsapp-member-admin",".member-lifecycle-toggle",".member-directory-list",".member-detail-dialog"])assert.ok(cleanup.includes(feature),`${feature} fehlt im aufgeräumten Mitgliederlayout`);
  for(const feature of ["MemberDetailDialog","Name oder Mitgliedsnummer suchen","Zugang fehlt","Funktionen bearbeiten","RFID-Karten verwalten","Buchungen und Zahlungen werden hier nur angezeigt"])
    assert.ok(page.includes(feature),`${feature} fehlt in der kompakten Mitgliederverwaltung`);
  assert.ok(page.includes('setDirectoryFilter(filter.id)')&&page.includes('setDetailMember(m)'),"Filter oder anklickbare Mitgliederzeilen fehlen");
});
...
test("entfernt den aktuellen Kunden für einen spontanen Gastverkauf",async()=>{
  ...
  for(const feature of ["clearBillingMember","× Auswahl entfernen",'setActiveMember(null)','setListMember(null)','setMemberPricing(false)','sessionStorage.removeItem("vereinskasse-active-member")'])assert.ok(page.includes(feature),`${feature} fehlt`);
